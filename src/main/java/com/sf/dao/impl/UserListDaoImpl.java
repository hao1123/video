...
public class UserListDaoImpl implements UserListDao {
	...
	public videoEntity videoInfo(String videoID) {
		String sql = "select * from video where videoID = ?";
		videoEntity video = jdbcTemplate.getJdbcTemplate().queryForObject(sql, new Object[]{videoID}, new VideoRowMapperEntity());
		return video;
	}
	
	
	public int upLoad(videoEntity video){
		int num = 0;
		String sql = "insert into video(videoName,videoImage,videoAddress,videolookTime,videoCollection,videoLeaving,videoTime,videoState,videocAtegory) value(?,?,?,?,?,?,?,?,?)";
		num = jdbcTemplate.getJdbcTemplate().update(sql, new Object[]{video.getVideoName(),video.getVideoImage(),video.getVideoAddress(),video.getVideolookTime(),video.getVideoCollection(),video.getVideoLeaving(),video.getVideoTime(),video.getVideoState(),video.getVideocAtegory()});
		return num;
